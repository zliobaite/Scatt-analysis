...
outputFile.WriteLine "Shot number" & "The moment of shot" & "Shot result" _
	& "Aiming time" & "Breach coordinate X (mm)" & "Breach coordinate Y (mm)" _
		& "Average aiming point X (mm)" & "Average aiming point Y (mm)" _
			& "Steadiness in 10.0 (%)" & "Steadiness in 10a0 (%)" _
				& "Trace length (mm)" & "Distance between breach and AAP"
'
...
For i = 1 To AllShots.Count
	...
	' store data
	'
	outputFile.WriteLine i & CurrentShot.ShotTime & CurrentShot.Result _
		& CurrentShot.ShotTime - CurrentShot.Attr.EnterTime & CurrentShot.BreachX _
			& - CurrentShot.BreachY & AverageX & - AverageY _
				& TestRange.Steadiness(Radius10, CurrentShot.Attr.FCoefficient, 0, 0) _
					& TestRange.Steadiness(Radius10, CurrentShot.Attr.FCoefficient, AverageX, AverageY) _
						& TestRange.Length & Sqr(dx*dx + dy*dy)
Next